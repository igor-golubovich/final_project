        ...
        steps {
          script {
            catchError (buildResult: 'SUCCESS', stageResult: 'FAILURE') {
              try {
                sh """
                echo "\$(date) " ${env.JOB_NAME} [${env.BUILD_NUMBER}] >> kubeval_db.log
                kubeval --strict --schema-location https://raw.githubusercontent.com/yannh/kubernetes-json-schema/master/ deploy/mysql.yaml >> kubeval_db.log
                echo "\n" >> kubeval_db.log
                """
                archiveArtifacts artifacts: 'kubeval_db.log'
          ...
          catchError (buildResult: 'SUCCESS', stageResult: 'FAILURE') {   
            try {
              if (sh(returnStdout: true, script: 'kubectl get deployment wordpress-mysql --ignore-not-found --namespace default --kubeconfig=$ckube') == '') {
                ...
                sh "kubectl set image deployment/wordpress-mysql mysql=$image_bd --namespace default --kubeconfig=$ckube"
...
stage("Rollback") {
 ...
 post {
    success {
        slackSend (color: '#00FF00', message: "SUCCESSFUL: DB Pipeline '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    }
    failure {
        slackSend (color: '#FF0000', message: "FAILED: DB Pipeline '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    }
  }
